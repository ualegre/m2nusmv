...
public class M2NuSMV {
	...
	List<RuleElement> auxiliaryStates = getAuxiliaryStates();
	for (RuleElement state : auxiliaryStates) {
	    writeInitialisation(state.getName() + "_aux", state.getStatus().toUpperCase());
	}
    ...
    private List<RuleElement> getAuxiliaryStates() {
	List<RuleElement> auxiliaryStates = new ArrayList<>();
	for (Rule rule : data.getStrs()) {
	    auxiliaryStates.add(rule.getConsequent());
	}
	...
	case STRONG_IMMEDIATE_PAST:
	    writeOperator("sip", "strong_immediate_past", bop.getStateName(), bop.getLowBound());
	    break;
	case WEAK_IMMEDIATE_PAST:
	    writeOperator("wip", "weak_immediate_past", bop.getStateName(), bop.getLowBound());
	    break;
	case STRONG_ABSOLUTE_PAST:
	    writeOperator("sap", "strong_absolute_past", bop.getStateName(),
		    bop.getLowBound() + "," + bop.getUppBound() + ",time");
	    break;
	case WEAK_ABSOLUTE_PAST:
	    writeOperator("wap", "weak_absolute_past", bop.getStateName(),
		    bop.getLowBound() + "," + bop.getUppBound() + ",time");
	    break;
	default:
	    break;
    ...
    protected void writeAuxiliaryStateVariables(RuleElement state) throws IOException {
